---
layout: place
title:  "Odell"
categories: places
lat: 52.20840823586665
lng: -0.591779254626702
background: /assets/images/1/odell_church.jpg
---
Odell is a village of around 200 people which apparently takes its name from the [woad](https://en.wikipedia.org/wiki/Isatis_tinctoria) that was gathered nearby.
<div id="map" class="col-md-12" style="height: 400px;"></div> 

Any erstwhile dying industry has gone the same way as the castle that used to be here, but what has been left behind is a substantial charch and a pub.

### Accommodation
The best options for accomodation are in Harrold which can be found on [booking.com](https://www.booking.com/searchresults.en-gb.html?ss=Harrold%2C+Bedfordshire%2C)

### Getting there
Odell is the other end of the Harrold Odell Country Park and best connections go from there to Bedford.
The No 25 bus from Harrold goes to Bedford via Stevington and [Pavenham](./pavenham.html), which is on the walking route.